...
extern std::unordered_set<std::string> processedPaths;
...
class string_parser
{
	...
	bool processContents( unsigned char* buffer, long numRead, LPCSTR filepath, string process);
public:
	string_parser( STRING_OPTIONS options );
	bool parse_block( unsigned char* buffer, unsigned int buffer_length, LPCSTR datasource, string process);
